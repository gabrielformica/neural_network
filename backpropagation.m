...
function [newWih, newWho] = backpropagation(valuesi, target, Wih, Who, rate)
    ...
    [deltai, deltah] = backpropagate(Wih, Who, deltao, valuesh);
    [newWih, newWho] = updateweights(Wih, Who, valuesi, valuesh, valueso, deltah, deltao, rate);
endfunction

function [newWih, newWho] = updateweights(Wih, Who, valuesi, valuesh, valueso, deltah, deltao, rate)
...
function [deltai, deltah] = backpropagate(Wih, Who, deltao, valuesh)
    deltah = valuesh' .* (1 - valuesh') .* (Who * deltao);
    deltai = Wih * deltah;    % <---------- ?
endfunction
